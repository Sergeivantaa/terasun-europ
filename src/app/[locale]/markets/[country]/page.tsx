      ...
      <section className="container-page py-12 md:py-16">
        <div className="stag mb-12">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-gold text-xs font-bold tracking-widest uppercase">{t('eyebrow')}</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-16">
          <div className="lg:col-span-2">
            ...
            <p className="text-gray-400 text-sm leading-relaxed mb-6">{t(`${country}.body2`)}</p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
              {['certifications', 'delivery', 'support', 'partner'].map(key => (
                <div key={key} className="card-gold p-4">
                  <h3 className="text-white font-semibold text-sm mb-1">{t(`country_features.${key}.title`)}</h3>
                  <p className="text-gray-400 text-xs">{t(`country_features.${key}.body`)}</p>
                </div>
              ))}
            </div>
          ...
          <aside className="space-y-4">
            <div className="card-gold p-6">
              <h3 className="font-bold text-white mb-4">{t('marketInfo')}</h3>
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-400">Country</span>
                  <span className="text-white font-medium">{c.flag} {c.name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Status</span>
                  <span className={c.status === 'active' ? 'text-green-400' : c.status === 'open' ? 'text-blue-400' : 'text-gold'}>{c.status}</span>
                </div>
                {c.port && (
                  <div className="flex justify-between">
            ...
            <div className="card-gold p-4">
              <h3 className="font-bold text-white text-sm mb-3">Other markets</h3>
