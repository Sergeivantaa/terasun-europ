    ...
    <>
      ...
      <section className="relative bg-darker overflow-hidden" id="top">
        ...
        <div className="absolute inset-0 pointer-events-none" aria-hidden="true"
          style={{ background: 'radial-gradient(ellipse 70% 60% at 30% 50%, rgba(180,83,9,0.08) 0%, transparent 70%), radial-gradient(ellipse 50% 40% at 80% 30%, rgba(245,158,11,0.05) 0%, transparent 60%)' }}
        />
        <div className="container-page relative py-20 lg:py-28">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <p className="animate-hero-1 text-xs font-semibold tracking-widest text-gold uppercase mb-4">
                {t('eyebrow')}
              </p>
              <h1 className="animate-hero-2 text-4xl sm:text-5xl font-black text-white leading-tight mb-6">
                {t('h1Line1')}<br/>
                <em className="not-italic text-gold2">{t('h1Line2Emphasis')}</em>{' '}
                {t('h1Line2')}<br/>
                {t('h1Line3')}
              </h1>
              <p className="animate-hero-3 text-lg text-gray-300 leading-relaxed mb-8 max-w-xl">
                {t('sub')}
              </p>
              ...
              <p className="animate-hero-5 text-xs text-gray-500 leading-relaxed">
                {t('note')}<br/>
                {t('noteManufacturer')}
              </p>
            </div>
            ...
            <div className="animate-hero-3">
              <div className="grid grid-cols-2 gap-3 mb-4">
                {[
                  ...
                  <div key={c.val} className="dark-card-lift bg-card-dark border border-border-dark rounded-xl p-4">
        ...
        <div className="container-page">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            ...
            <ScrollReveal delay={150}>
              ...
              <div className="bg-gold-light border border-gold-border rounded-xl p-5 mb-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="card-lift bg-page border border-gray-200 rounded-xl p-4 text-center">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {Object.values(auth.raw('cards') as Record<string, {title:string;body:string;cta?:string}>).map((card, i) => (
              <ScrollReveal key={card.title} delay={i * 80}>
                <div className="card-lift h-full bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
            ...
            <ScrollReveal>
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                {[
                  ...
                  <div key={s.label} className="dark-card-lift bg-card-dark border border-border-dark rounded-xl p-3 text-center">
          ...
          <ScrollReveal>
            ...
            <div className="grid lg:grid-cols-3 gap-4">
              {[
                ...
                <div key={s.label} className="card-lift bg-page border border-gray-200 rounded-xl p-5" style={{ transitionDelay: `${i * 60}ms` }}>
        ...
        <div className="container-page">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            {[
              ...
              <ScrollReveal key={app.slug} delay={i * 70}>
                <Link href={navHref(`/applications/${app.slug}`)} className="card-lift group block h-full bg-white border border-gray-200 rounded-xl p-5">
                  <div className="text-2xl mb-3">{app.icon}</div>
                  <h3 className="text-sm font-semibold text-gray-900 mb-2 group-hover:text-gold transition-colors duration-200">{apps(`${app.slug}.title`)}</h3>
        ...
        <div className="container-page">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[
              ...
              <ScrollReveal key={c.ref} delay={i * 80}>
                <div className="dark-card-lift h-full bg-card-dark border border-border-dark rounded-xl p-5">
      ...
      <section className="bg-white py-16 lg:py-20">
        <div className="container-page">
          <ScrollReveal>
            <div className="bg-dark border border-border-dark rounded-2xl p-8 lg:p-12 text-center overflow-hidden relative">
              {/* Subtle inner glow */}
              <div className="absolute inset-0 pointer-events-none" aria-hidden="true"
                style={{ background: 'radial-gradient(ellipse 60% 50% at 50% 0%, rgba(245,158,11,0.06) 0%, transparent 70%)' }}
              />
              <p className="stag text-center relative">{home('distStag')}</p>
              <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4 relative">{home('distTitle')}</h2>
              ...
              <div className="flex flex-wrap justify-center gap-2 mb-8 relative">
                {['Germany', 'Netherlands', 'France', 'Poland', 'Czech Republic', 'Spain', 'Italy', 'Austria'].map((c) => (
                  <span key={c} className="text-xs bg-card-dark border border-border-dark text-gray-400 px-3 py-1.5 rounded-full hover:border-gold/30 hover:text-gray-300 transition-colors">{c} — {home('distCountryLabel')}</span>
                ))}
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-border-dark relative">
            ...
            <div className="space-y-3">
              {[
                ...
                <ScrollReveal key={c.label} delay={i * 80}>
                  <div className="dark-card-lift bg-card-dark border border-border-dark rounded-xl p-4 flex items-center gap-4">
                    <div className="text-xs font-bold tracking-wider text-gray-500 uppercase w-16 shrink-0">{c.label}</div>
                    {c.href
                      ? <a href={c.href} className="text-sm text-white hover:text-gold2 transition-colors duration-150">{c.val}</a>
