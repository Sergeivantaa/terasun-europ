    ...
    <>
      ...
      <section className="container-page py-12 md:py-16">
          ...
          {steps.map((step, i) => (
            <div key={step} className="card-gold p-6">
              <div className="flex items-center gap-3 mb-4">
                <span className="w-8 h-8 rounded-full bg-gold/20 border border-gold-border flex items-center justify-center text-gold font-bold text-sm shrink-0">
                  {i + 1}
                </span>
                <h3 className="font-bold text-white">{t(`steps.${step}.title`)}</h3>
              </div>
              <p className="text-gray-400 text-sm leading-relaxed">{t(`steps.${step}.body`)}</p>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-8 mb-12">
          <h2 className="text-xl font-bold text-white mb-6">{t('leadTimesTitle')}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {['stock', 'production', 'custom'].map(lt => (
              <div key={lt} className="text-center">
                <p className="text-3xl font-black text-gold2 mb-1">{t(`leadTimes.${lt}.value`)}</p>
                <p className="text-white text-sm font-semibold mb-1">{t(`leadTimes.${lt}.label`)}</p>
                <p className="text-gray-400 text-xs">{t(`leadTimes.${lt}.desc`)}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Incoterms */}
        <div className="mb-12">
          <h2 className="text-xl font-bold text-white mb-4">{t('incotermsTitle')}</h2>
          <div className="flex flex-wrap gap-3">
            {['EXW', 'FOB', 'CFR', 'CIF', 'DAP', 'DDP'].map(term => (
              <span key={term} className="cert-badge text-sm px-4 py-1.5">{term}</span>
            ))}
          </div>
          <p className="text-gray-400 text-sm mt-3">{t('incotermsNote')}</p>
        </div>
